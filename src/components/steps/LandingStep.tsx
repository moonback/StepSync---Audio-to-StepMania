...
export const LandingStep: React.FC<LandingStepProps> = ({ onStart, onShowHelp }) => {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center justify-center min-h-[70vh] w-full max-w-5xl px-4"
    >
      <div className="relative sm-panel sm-scanlines rounded-[3rem] p-12 sm:p-20 w-full overflow-hidden text-center">
        ...
        <div className="relative z-10 mb-12 flex flex-col items-center">
          ...
          <p className="text-[10px] sm:text-xs font-black text-[#00f5ff]/60 uppercase tracking-[0.5em] mb-10">
            Générateur de pas pour StepMania v2.0
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-16 relative z-10">
          {[
            { icon: <Music2 className="w-10 h-10 text-[#e83f9a]" />, title: 'ANALYSE DSP', desc: 'Détection de BPM chirurgicale' },
            { icon: <Cpu className="w-10 h-10 text-[#27e86b]" />, title: 'AI CHOREO', desc: 'Génération de patterns pro' },
            { icon: <Disc className="w-10 h-10 text-[#f5e542]" />, title: 'PACK ZIP', desc: 'Exportation prête à jouer' }
          ].map((feat, i) => (
            <motion.div
              key={feat.title}
